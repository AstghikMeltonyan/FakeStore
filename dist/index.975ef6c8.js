...
const router = new (0, _navigoDefault.default)("/"); // es mer glxavor ejn a, ete uzum enq gnal urish ej  sleshic heto kgrenq eji anune
...
function catalogDetalis(id) {
    ...
    return (0, _redom.el)("div", [
        (0, _redom.el)("h1", `Product detalis ${id}`),
        body
    ]);
}
router.on("/", ()=>{
...
router.on("/product/:id", ({ data: { id } })=>{
    (0, _redom.setChildren)(window.document.body, catalogDetalis(id));
});
router.resolve();
...
},{}]},["igKGj","8lqZg"], "8lqZg", "parcelRequirefeec")
